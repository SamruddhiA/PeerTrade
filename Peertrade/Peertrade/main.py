...
def gen_custID():
    conn = sqlite3.connect("Peertrade/peertrade.db")
    cur = conn.cursor()
    cur.execute("UPDATE metadata SET custnum = custnum + 1")
    conn.commit()
    custnum = str([i for i in cur.execute(
        "SELECT custnum FROM metadata")][0][0])
    conn.close()
...
def gen_sellID():
    ...
    conn.commit()
    sellnum = str([i for i in cur.execute(
        "SELECT sellnum FROM metadata")][0][0])
    conn.close()
...
def gen_prodID():
    ...
    conn.commit()
    prodnum = str([i for i in cur.execute(
        "SELECT prodnum FROM metadata")][0][0])
    conn.close()
...
def gen_orderID():
    ...
    conn.commit()
    ordernum = str([i for i in cur.execute(
        "SELECT ordernum FROM metadata")][0][0])
    conn.close()
...
def auth_user(data):
    ...
    if type == "Customer":
        a = cur.execute(
            "SELECT custID, name FROM customer WHERE email=? AND password=?", (email, password))
    elif type == "Seller":
        a = cur.execute(
            "SELECT sellID, name FROM seller WHERE email=? AND password=?", (email, password))
    a = list(a)
    conn.close()
    if len(a) == 0:
        return False
...
def search_users(search, srch_type):
    ...
    if srch_type == "Customer":
        res = cur.execute(
            "SELECT custID, name, email, phone, area, locality, city, state, country, zipcode FROM customer WHERE LOWER(name) like ?", (search,))
    elif srch_type == "Seller":
        res = cur.execute(
            "SELECT sellID, name, email, phone, area, locality, city, state, country, zipcode FROM seller WHERE LOWER(name) like ?", (search,))
    res = [i for i in res]
    conn.close()
    return res
...
def check_psswd(psswd, userid, type):
    ...
    if type == "Customer":
        a = cur.execute(
            "SELECT password FROM customer WHERE custID=?", (userid,))
    elif type == "Seller":
        a = cur.execute(
            "SELECT password FROM seller WHERE sellID=?", (userid,))
...
def set_psswd(psswd, userid, type):
    ...
    if type == "Customer":
        ...
        a = cur.execute(
            "UPDATE seller SET password=? WHERE sellID=?", (psswd, userid))
    conn.commit()
    conn.close()
...
def add_prod(sellID, data):
    ...
    tup = (prodID,
           ...
           sellID)
    cur.execute(
        "INSERT INTO product VALUES (?,?,?,?,?,(SELECT profit_rate from metadata)*?,?,?)", tup)
    conn.commit()
    conn.close()
...
def get_categories(sellID):
    ...
    cur = conn.cursor()
    a = cur.execute(
        "SELECT DISTINCT(category) from product where sellID=?", (sellID,))
...
def get_seller_products(sellID):
    ...
    cur = conn.cursor()
    a = cur.execute(
        "SELECT prodID, name, category, sell_price FROM product WHERE sellID=? AND quantity!=0", (sellID,))
    res = [i for i in a]
    conn.close()
    return res
...
def update_cart(custID, qty):
    ...
    cur = conn.cursor()
    for prodID in qty:
        cur.execute("DELETE FROM cart WHERE prodID=? AND custID=?",
                    (prodID, custID))
        cur.execute("INSERT INTO cart VALUES (?,?,?)",
                    (custID, prodID, qty[prodID]))
    conn.commit()
    conn.close()
...
def cart_purchase(custID):
    ...
    for item in cart:
        ...
        cur.execute("""INSERT INTO orders
                        SELECT ?,?,?,?,datetime('now'), cost_price*?, sell_price*?, 'PLACED'
                        FROM product WHERE prodID=? """, (orderID, custID, prodID, qty, qty, qty, prodID))
        cur.execute("DELETE FROM cart WHERE custID=? AND prodID=?",
                    (custID, prodID))
        conn.commit()
    conn.close()
...
def view_profile(id):
    ...
    return render_template("view_profile.html",
                           ...
                           country=det[8],
                           zip=det[9],
                           category=(None if profile_type ==
                                     "Customer" else categories),
                           my=my)
...
def my_products():
    ...
    if request.method == "POST":
        ...
        results = search_myproduct(
            session['userid'], srchBy, category, keyword)
...
def view_product(id):
    ...
    if not ispresent:
        abort(404)
    (name, quantity, category, cost_price,
     sell_price, sellID, desp, sell_name) = tup
